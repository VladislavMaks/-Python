...
while True:
    question = input('Добавить товар да/нет? ')
    if question.upper() == 'НЕТ':
        break
    item = {}
    for spe in spec:
        item[spe] = input(f'Введите {spe} ')
...
result_dict = {}
for item in result:
    for my_key, my_value in item[1].items():
        if my_key in result_dict:
            result_dict[my_key].append(my_value)
        else:
            result_dict[my_key] = [my_value]
print(result_dict)
